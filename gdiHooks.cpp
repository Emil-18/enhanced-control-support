...
# include <vsstyle.h>
#include <vssym32.h>
// Fore some reason, the OpenThemeData and DrawThemeBackground functions gives a linker error when they are used directly in the code.
// So define them manualy.
typedef HTHEME(WINAPI* OpenThemeData_funcType)(HWND, LPCWSTR);
typedef HRESULT(WINAPI* DrawThemeBackground_funcType)(HTHEME, HDC, int, int, LPCRECT, LPCRECT);
...
// To work around this, start this version of NVDA while the program runs
# define LOGFUNCTIONS 1
#if LOGFUNCTIONS
#define LOGFUNCTIONINFO(message) LOG_INFO(message)
#else
#define LOGFUNCTIONINFO(message)
#endif // LOGFUNCTIONS
...
void ExtTextOutHelper(
	...
	int cbCount, LPSIZE resultTextSize, int direction, const wstring originalFunctionName = L"No function name provided"
) {
	const bool isOpaqueRequested = fuOptions & ETO_OPAQUE; // The current background color should be used to fill the rectangle.
	LOGFUNCTIONINFO(originalFunctionName << L" called with arguments model: " << model << L", hdc: " << hdc << L"Window or bitmap:" <<  (WindowFromDC(hdc) ? WindowFromDC(hdc) : GetCurrentObject(hdc, OBJ_BITMAP)) << L", x: " << x << L" y: " << y << L", LPRC: " << lprc << L", textalline: " << textAlign << L", stripHotkeyIndicator: " << stripHotkeyIndicator << L", lpString: " << (lpString ? lpString : L" ") << L", direction: " << direction << L", isOpaqueRequested: " << isOpaqueRequested);
	...
	*resultTextSize = textSize; // resultTextSize is an out-param and must be calculated before early exit.
	...
	LOGFUNCTIONINFO(L"After convertion to screen coordinates, left top right bottom: " << textRect.left << L" " << textRect.top << L" " << textRect.right << L" " << textRect.bottom);
	...
	if(cbCount>0&&tm.tmCharSet!=SYMBOL_CHARSET) {
		...
		LOGFUNCTIONINFO(L"Inserting final text: " << newText);
 ...
 * @param codePage the code page used for the string which will be converted to unicode
  */
void ExtTextOutHelper(displayModel_t* model, HDC hdc, int x, int y, const RECT* lprc,UINT fuOptions,UINT textAlign, BOOL stripHotkeyIndicator, const char* lpString, const int codePage, const int* lpdx, int cbCount, LPSIZE resultTextSize, int direction, const wstring originalFunctionName = L"No function name provided") {
	int newCount=0;
...
template<typename charType> int  WINAPI hookClass_TextOut<charType>::fakeFunction(HDC hdc, int x, int y, const charType* lpString, int cbCount) {
	...
	wstring functionName = L"TextOut";
	ExtTextOutHelper(model,hdc,pos.x,pos.y,NULL,0,textAlign,FALSE,lpString,CP_THREAD_ACP,NULL,cbCount,NULL,false, functionName);
	model->release();
	return res;
}
...
template<typename WA_POLYTEXT> BOOL WINAPI hookClass_PolyTextOut<WA_POLYTEXT>::fakeFunction(HDC hdc,const WA_POLYTEXT* pptxt,int cStrings) {
	...
	for(int i=0;i<cStrings;++i) {
		...
		//record the text
		ExtTextOutHelper(model,hdc,curPos.x,curPos.y,&curClearRect,curPptxt->uiFlags,textAlign,FALSE,curPptxt->lpstr,CP_THREAD_ACP,curPptxt->pdx,curPptxt->n,&curTextSize,false, L"PolyTextOut");
...
int WINAPI fake_FillRect(HDC hdc, const RECT* lprc, HBRUSH hBrush) {
	...
	LOGFUNCTIONINFO(L"FillRect called. hdc: " << hdc << L", model: " << model << L", rect left top right bottom: " << rect.left << L" " << rect.top << L" " << rect.right << L" " << rect.bottom);
...
BOOL WINAPI fake_PatBlt(HDC hdc, int nxLeft, int nxTop, int nWidth, int nHeight, DWORD dwRop) {
	...
	dcPointsToScreenPoints(hdc,(LPPOINT)&rect,2,false);
	LOGFUNCTIONINFO(L"PatBlt called. hdc: " << hdc << L", model: " << model << L", rect left top right bottom: " << rect.left << L" " << rect.top << L" " << rect.right << L" " << rect.bottom);
	model->clearRectangle(rect);
	model->release();
	return res;
}
...
template<typename charType> BOOL __stdcall hookClass_ExtTextOut<charType>::fakeFunction(HDC hdc, int x, int y, UINT fuOptions, const RECT* lprc, const charType* lpString, UINT cbCount, const INT* lpDx) {
	...
	ExtTextOutHelper(model,hdc,pos.x,pos.y,lprc,fuOptions,textAlign,FALSE,lpString,CP_THREAD_ACP,lpDx,cbCount,NULL,direction, L"ExtTextOut");
...
DrawThemeBackground_funcType real_DrawThemeBackground = NULL;
HRESULT WINAPI fake_DrawThemeBackground(HTHEME theme, HDC hdc, int partId, int stateId, LPCRECT rect, LPCRECT clippingRect) {
	HRESULT res = real_DrawThemeBackground(theme, hdc, partId, stateId, rect, clippingRect);
	if (res != S_OK) {
		return(res);
	}
	if (partId != BP_CHECKBOX ) {
		return(res);
	}
...
void StretchBlt_helper(HDC hdcDest, int nXDest, int nYDest, int nWidthDest, int nHeightDest, HDC hdcSrc, int nXSrc, int nYSrc, int nWidthSrc, int nHeightSrc, DWORD dwRop, const wstring originalFunctionName = L"No function name provided") {
	...
	if(useSource) {
		srcModel=acquireDisplayModel(hdcSrc,FALSE);
	}
	...
	LOGFUNCTIONINFO(  originalFunctionName <<  L" called with arguments: hdcDest, " << hdcDest << L", window or bitmap: " << (WindowFromDC(hdcDest) ? WindowFromDC(hdcDest) : GetCurrentObject(hdcDest, OBJ_BITMAP)) << L", nxDest: " << nXDest << L", nyDest: " << nYDest << L", nWidthDest: " << nWidthDest << L", nHeightDest: " << nHeightDest << L", hdcSrc: " << hdcSrc << L", window or bitmap: " << (WindowFromDC(hdcSrc) ? WindowFromDC(hdcSrc) : GetCurrentObject(hdcSrc, OBJ_BITMAP)) << L", nxSrc: " << nXSrc << L", nySrc: "<< L", " << nYSrc << L", nWidthSrc: " << nWidthSrc  << L", nHeightSrc: " << nHeightSrc  << L", dwRop: " << dwRop << L". The display models are dest and src models respectivly, " << destModel << L"with chunk count of "<< destModel->getChunkCount()  <<L" and " << srcModel << L"with a chunk count of " << srcModel->getChunkCount());
	...
	if (srcModel) {
		LOGFUNCTIONINFO(L"Source after convertion to screen coordinates, left top right bottom: " << srcRect.left << L" " << srcRect.top << L" " << srcRect.right << L" " << srcRect.bottom);
	}
	LOGFUNCTIONINFO(L"Dest after convertion to screen coordinates, left top right bottom: " << destRect.left << L" " << destRect.top << L" " << destRect.right << L" " << destRect.bottom);
	if(destInvertBefore) {
	...
	if(srcModel) {
		...
		// When we store memory models by bitmaps instead of dcs, some text, such as in the run dialog, gets scrambeled.
		// This may happen because the application draws text to one window, uses the same bitmap to draw to another window, and clears the previously drawn text via a method we don't detect
	...
	//if (srcModel && srcModel != destModel) srcModel->clearAll();
...
InternalBitBlt_funcType InternalBitBlt = (InternalBitBlt_funcType)*address;
InternalBitBlt_funcType real_InternalBitBlt = NULL;
BOOL APIENTRY fake_InternalBitBlt(HDC hdc, INT x, INT y, INT width, INT height, INT xSrc, INT ySrc, DWORD dwRop) {
	//Beep(500, 10);
	BOOL res = real_InternalBitBlt(hdc, x, y, width, height, xSrc, ySrc, dwRop);
	if (!res) return(res);
	StretchBlt_helper(hdc, x, y, width, height, NULL, xSrc, ySrc, width, height, dwRop, L"InternalBitBlt");
	return(res);
}
typedef BOOL(WINAPI* MaskBlt_funcType)(HDC, int, int, int, int, HDC, int, int, HBITMAP, int, int, DWORD);
MaskBlt_funcType real_MaskBlt = NULL;
BOOL WINAPI fake_MaskBlt(HDC hdcDest, int nXDest, int nYDest, int nWidth, int nHeight, HDC hdcSrc, int nXSrc, int nYSrc, HBITMAP bitmap, int xMask, int yMask, DWORD dwRop) {
	//Call the real BitBlt
	BOOL res = real_MaskBlt(hdcDest, nXDest, nYDest, nWidth, nHeight, hdcSrc, nXSrc, nYSrc, bitmap, xMask, yMask, dwRop);
	//If bit blit didn't work, or its not a simple copy, we don't want to know about it
	if (!res) return res;
	//Beep(500, 50);
	StretchBlt_helper(hdcDest, nXDest, nYDest, nWidth, nHeight, hdcSrc, nXSrc, nYSrc, nWidth, nHeight, dwRop, L"MaskBlt");
	return res;
}


typedef BOOL(WINAPI* PlgBlt_funcType)(HDC, const POINT*, HDC, int, int, int, int, HBITMAP, int, int);
PlgBlt_funcType real_PlgBlt = NULL;
BOOL WINAPI fake_PlgBlt(HDC hdcDest, const POINT* points, HDC hdcSrc, int srcX, int srcY, int srcWidth, int srcHeight, HBITMAP bitmap, int maskX, int maskY) {
	//Beep(500, 50);
	BOOL res = real_PlgBlt(hdcDest, points, hdcSrc, srcX, srcY, srcWidth, srcHeight, bitmap, maskX, maskY);
	if (!res) return(res);
	//Beep(500, 50);
	int xDest = points[0].x;
	int yDest = points[0].y;
	int widthDest = points[1].x - points[0].x;
	int heightDest = points[2].y - points[0].y;
	StretchBlt_helper(hdcDest, xDest, yDest, widthDest, heightDest, hdcSrc, srcX, srcY, srcWidth, srcHeight, SRCPAINT, L"PlgBlt");
	return(res);
}
typedef BOOL(WINAPI* GdiAlphaBlend_funcType)(HDC, int, int, int, int, HDC, int, int, int, int, BLENDFUNCTION);
GdiAlphaBlend_funcType real_GdiAlphaBlend = NULL;
BOOL WINAPI fake_GdiAlphaBlend(HDC hdcDest, int nXDest, int nYDest, int nWidthDest, int nHeightDest, HDC hdcSrc, int nXSrc, int nYSrc, int nWidthSrc, int nHeightSrc, BLENDFUNCTION crTransparent) {
	//Call the real StretchBlt
	BOOL res = real_GdiAlphaBlend(hdcDest, nXDest, nYDest, nWidthDest, nHeightDest, hdcSrc, nXSrc, nYSrc, nWidthSrc, nHeightSrc, crTransparent);
	if (!res) return res;
	//Beep(500, 50);
...
BOOL WINAPI fake_StretchBlt(HDC hdcDest, int nXDest, int nYDest, int nWidthDest, int nHeightDest, HDC hdcSrc, int nXSrc, int nYSrc, int nWidthSrc, int nHeightSrc, DWORD dwRop) {
	...
	StretchBlt_helper(hdcDest, nXDest, nYDest, nWidthDest, nHeightDest, hdcSrc, nXSrc, nYSrc, nWidthSrc, nHeightSrc, dwRop, L"StretchBlt");
	return res;
}
...
BOOL WINAPI fake_GdiTransparentBlt(HDC hdcDest, int nXDest, int nYDest, int nWidthDest, int nHeightDest, HDC hdcSrc, int nXSrc, int nYSrc, int nWidthSrc, int nHeightSrc, UINT crTransparent) {
	...
	if(!res) return res;
	StretchBlt_helper(hdcDest, nXDest, nYDest, nWidthDest, nHeightDest, hdcSrc, nXSrc, nYSrc, nWidthSrc, nHeightSrc, SRCPAINT, L"GdiTransparentBlt");
	return res;
}
...
void gdiHooks_inProcess_initialize() {
	...
	apiHook_hookFunction_safe(TextOutA, hookClass_TextOut<char>::fakeFunction, &hookClass_TextOut<char>::realFunction);//logged
	apiHook_hookFunction_safe(TextOutW, hookClass_TextOut<wchar_t>::fakeFunction, &hookClass_TextOut<wchar_t>::realFunction);//logged
	apiHook_hookFunction_safe(PolyTextOutA, hookClass_PolyTextOut<POLYTEXTA>::fakeFunction, &hookClass_PolyTextOut<POLYTEXTA>::realFunction);//logged
	apiHook_hookFunction_safe(PolyTextOutW, hookClass_PolyTextOut<POLYTEXTW>::fakeFunction, &hookClass_PolyTextOut<POLYTEXTW>::realFunction);//logged
	apiHook_hookFunction_safe(ExtTextOutA, hookClass_ExtTextOut<char>::fakeFunction, &hookClass_ExtTextOut<char>::realFunction);//logged
	apiHook_hookFunction_safe(ExtTextOutW, hookClass_ExtTextOut<wchar_t>::fakeFunction, &hookClass_ExtTextOut<wchar_t>::realFunction);//logged
	...
	apiHook_hookFunction_safe(GdiAlphaBlend, fake_GdiAlphaBlend, &real_GdiAlphaBlend);
	...
	//apiHook_hookFunction_safe(drawThemeBackground, fake_DrawThemeBackground, &real_DrawThemeBackground);
